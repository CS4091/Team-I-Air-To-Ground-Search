...
class Drone:
    ...
    def move_straight(self):
        if self.direction == Direction.UP and self.row > 0 and self.grid[self.row - 1, self.col] == 0:
            self.row -= 1
        elif self.direction == Direction.DOWN and self.row < self.grid.shape[0] - 1 and self.grid[self.row + 1, self.col] == 0:
            self.row += 1
        elif self.direction == Direction.LEFT and self.col > 0 and self.grid[self.row, self.col - 1] == 0:
            self.col -= 1
        elif self.direction == Direction.RIGHT and self.col < self.grid.shape[1] - 1 and self.grid[self.row, self.col + 1] == 0:
            self.col += 1
...
if __name__ == "__main__":
    grid_filepath = pathlib.Path("grid_world.csv")
